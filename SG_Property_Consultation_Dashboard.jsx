...
// ════════════════════════════════════════════════════════════
// CALCULATION ENGINE (inline — matches sg-property-engine.js)
// ════════════════════════════════════════════════════════════

const DEFAULTS = {
  mortgageRate: 0.015, stressTestRate: 0.04, investmentRate: 0.038,
  loanTenure: 30, ltv1st: 0.75, ltv2nd: 0.45,
  absd: { SC: [0, 0.20, 0.35], PR: [0.05, 0.30, 0.35], Foreigner: [0.60, 0.60, 0.60] },
  legalFees: 3800, renoFurnishing: 20000, agentCommSell: 0.01,
  occupancyRate: 0.90, avPctOfRent: 0.85, propTaxRateNROC: 0.11,
...
function calcBSD(price) {
  const b = [{ l: 180000, r: 0.01 }, { l: 180000, r: 0.02 }, { l: 640000, r: 0.03 }, { l: 500000, r: 0.04 }, { l: 1500000, r: 0.05 }, { l: Infinity, r: 0.06 }];
  let rem = price, bsd = 0;
  for (const { l, r } of b) { const t = Math.min(rem, l); bsd += t * r; rem -= t; if (rem <= 0) break; }
  return Math.round(bsd);
}
function calcABSD(price, res = "SC", count = 0) { return Math.round(price * (DEFAULTS.absd[res] || DEFAULTS.absd.SC)[Math.min(count, 2)]); }
function calcPMT(principal, rate, years = 30) {
  if (principal <= 0) return 0; if (rate <= 0) return principal / (years * 12);
  const r = rate / 12, n = years * 12;
  return principal * (r * Math.pow(1 + r, n)) / (Math.pow(1 + r, n) - 1);
}
function calcRemLoan(principal, rate, tenure, elapsed) {
  if (principal <= 0) return 0; if (rate <= 0) return principal * (1 - elapsed / tenure);
  const r = rate / 12, n = tenure * 12, p = elapsed * 12;
  return Math.max(0, principal * (Math.pow(1 + r, n) - Math.pow(1 + r, p)) / (Math.pow(1 + r, n) - 1));
}
function solveIRR(cf, guess = 0.08) {
  let rate = guess;
  for (let i = 0; i < 100; i++) {
    let npv = 0, d = 0;
    for (let t = 0; t < cf.length; t++) { npv += cf[t] / Math.pow(1 + rate, t); if (t > 0) d -= t * cf[t] / Math.pow(1 + rate, t + 1); }
    if (Math.abs(d) < 1e-12) break;
    const nr = rate - npv / d; if (Math.abs(nr - rate) < 1e-7) return nr; rate = nr;
  }
  return rate;
}
...
// FORMAT HELPERS
// ════════════════════════════════════════════════════════════
const f$ = n => n != null ? `$${Math.round(n).toLocaleString("en-SG")}` : "-";
const f$k = n => n != null ? `$${Math.round(n/1000)}K` : "-";
const f$m = n => n != null ? `$${(n/1e6).toFixed(2)}M` : "-";
const fPct = n => n != null ? `${(n*100).toFixed(1)}%` : "-";
const fCarry = n => { if (n == null) return "-"; return n >= 0 ? `+${f$(n)}/mo` : `-${f$(Math.abs(n))}/mo`; };
...
function ScenarioTab({ profile }) {
  const [selectedInvProject, setSelectedInvProject] = useState(1); // For dynamic Scenario C
  ...
  const scenarios = useMemo(() => {
    ...
    const upgMtg = calcPMT(upgLoan, DEFAULTS.mortgageRate);
    ...
    const upgRemLoan5 = calcRemLoan(upgLoan, DEFAULTS.mortgageRate, 30, 5);
    ...
    const invMtg = calcPMT(invLoan, DEFAULTS.investmentRate);
    ...
    if (cp.owns && (profile.residency === "SC" || profile.residency === "PR")) {
      ...
      const p1Mtg = calcPMT(p1Loan, DEFAULTS.investmentRate);
      const p2Mtg = calcPMT(p2Loan, DEFAULTS.investmentRate);
    ...
    if (cp.owns && cp.type?.includes("HDB")) {
      ...
      const hdbMtgBank = calcPMT(hdbLoan * 0.5, DEFAULTS.mortgageRate, 25); // Assume 50% via bank
...
function ShortlistTab({ profile }) {
  ...
  const enriched = useMemo(() => {
    ...
    return PROJECTS.map(p => {
      ...
      const mortgage = calcPMT(loan, DEFAULTS.investmentRate);
      ...
      const remLoan = calcRemLoan(loan, DEFAULTS.investmentRate, 30, 5);
      // Simple IRR estimate
...
function DeepDiveTab({ profile }) {
  ...
  const analysis = useMemo(() => {
    ...
    const remLoan5 = calcRemLoan(loan, DEFAULTS.investmentRate, 30, 5);
